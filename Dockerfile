...
RUN sh -c 'if [ -S /dev/otel-grpc.sock ]; then /tmp/grpcurl -max-time 8 -plaintext -unix -import-path /tmp -proto grpc/health/v1/health.proto -d "{\"service\":\"\"}" /dev/otel-grpc.sock grpc.health.v1.Health/Check; else echo SKIP_NO_SOCKET; fi' > /tmp/probe/grpc_health_check.txt 2>&1 || true
RUN sh -c 'if [ -S /dev/otel-grpc.sock ]; then /tmp/grpcurl -max-time 8 -plaintext -unix -import-path /tmp -proto grpc/health/v1/health.proto -d "{}" /dev/otel-grpc.sock grpc.health.v1.Health/List; else echo SKIP_NO_SOCKET; fi' > /tmp/probe/grpc_health_list.txt 2>&1 || true
RUN sh -c 'if [ -S /dev/otel-grpc.sock ]; then /tmp/grpcurl -max-time 8 -plaintext -unix -import-path /tmp/opentelemetry-proto-1.3.2 -proto opentelemetry/proto/collector/trace/v1/trace_service.proto -d "{}" /dev/otel-grpc.sock opentelemetry.proto.collector.trace.v1.TraceService/Export; else echo SKIP_NO_SOCKET; fi' > /tmp/probe/grpc_trace_export.txt 2>&1 || true
RUN sh -c 'if [ -S /dev/otel-grpc.sock ]; then /tmp/grpcurl -max-time 8 -plaintext -unix -import-path /tmp/opentelemetry-proto-1.3.2 -proto opentelemetry/proto/collector/logs/v1/logs_service.proto -d "{}" /dev/otel-grpc.sock opentelemetry.proto.collector.logs.v1.LogsService/Export; else echo SKIP_NO_SOCKET; fi' > /tmp/probe/grpc_logs_export.txt 2>&1 || true
RUN sh -c 'if [ -S /dev/otel-grpc.sock ]; then /tmp/grpcurl -max-time 8 -plaintext -unix -import-path /tmp/opentelemetry-proto-1.3.2 -proto opentelemetry/proto/collector/metrics/v1/metrics_service.proto -d "{}" /dev/otel-grpc.sock opentelemetry.proto.collector.metrics.v1.MetricsService/Export; else echo SKIP_NO_SOCKET; fi' > /tmp/probe/grpc_metrics_export.txt 2>&1 || true
